                                ...
                                ofstream& console_csv) const
{
   if (FLAGS_print_tx_console) {
      ...
      table.add_row({to_string(seconds), cr_table.get("0", "tx"), cr_table.get("0", "tx_abort"), cr_table.get("0", "gct_committed_tx"),
                     ...
                     cr_table.get("0", "gct_rounds"), to_string(free), to_string(part0), to_string(part1), "0", bm_table.get("0", "touches"), bm_table.get("0", "evicted_pages")});
      // -------------------------------------------------------------------------------------
...
void LeanStore::printStats(bool reset)
{
   if (reset) {
      ...
      cout << "total_cools: " << utils::threadlocal::sum_reset(PPCounters::pp_counters, &PPCounters::total_unswizzled) << endl;
   }else{
      cout << "no reset" << endl;
      ...
      cout << "total_cools: " << utils::threadlocal::sum_no_reset(PPCounters::pp_counters, &PPCounters::total_unswizzled) << endl;
   }
}
